...
from ..config import DRIVER_NAMES, F1_KEYWORDS, HIGH_PRIORITY_KEYWORDS, TEAM_NAMES
...
class BaseCollector(ABC):
    ...
    def calculate_relevance_score(self, title: str, content: str) -> float:
        ...
        title_priority_matches = sum(
            1 for keyword in HIGH_PRIORITY_KEYWORDS if keyword in title_text
        )
    ...
    def is_duplicate(self, title: str, content: str, existing_items: list[NewsItem]) -> bool:
        ...
        for item in existing_items:
            ...
            if self._calculate_similarity(content_lower[:200], item.content.lower()[:200]) > 0.7:
                return True
