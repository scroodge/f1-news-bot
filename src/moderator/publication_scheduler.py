...
class PublicationScheduler:
    ...
    def can_publish_now(self) -> bool:
        ...
        self.post_history = [
            post_time for post_time in self.post_history if now - post_time < timedelta(hours=1)
        ]

        # Check if we're under the limit
        return len(self.post_history) < self.max_posts_per_hour

    def get_next_publication_time(self) -> datetime | None:
        ...
        try:
            ...
            for i, item in enumerate(self.publication_queue):
                if item["priority"] < priority or (
                    ...
                    and item["scheduled_for"] > queue_item["scheduled_for"]
                ):
